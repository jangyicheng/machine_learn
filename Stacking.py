...
def parse_args():
    ...
    parser.add_argument("--threshold", type=float, default=0.4, help="Threshold for prediction.")
    parser.add_argument("--output_path", type=str, default="../commit/submission.csv",
                        help="Path to save prediction results.")
    parser.add_argument("--test_path", type=str, default="../data/valid_with_normalize_unique.csv", help="Path to test data.")
    parser.add_argument("--base_count", type=int, default=5, help="Number of base classifiers for each model.")
    parser.add_argument("--use_imbalance", type=int, default=1, help="Flag to enable imbalance handling.")
...
def load_and_preprocess_data(data_path, final_features):
    ...
    data = clean_feature_names(data).round(4)  # Adjust precision
...
def perform_grid_search(model, param_grid, X, y):
    grid_search = GridSearchCV(
        estimator=model, param_grid=param_grid, cv=6, n_jobs=-1,
...
def get_base_classifiers(X_train, y_train, run_flag, base_count, use_imbalance):
    # Set up hyperparameter grids with optional imbalance handling parameters
    param_grids = {
    ...
    # Set imbalance parameters based on user choice
    pos_weight = sum(y_train == 0) / sum(y_train == 1) if use_imbalance else None
    imbalance_params = {
        'XGB': {'scale_pos_weight': pos_weight} if use_imbalance else {},
        'LGB': {'is_unbalance': True} if use_imbalance else {},
        'CAT': {'class_weights': [1, pos_weight]} if use_imbalance else {}
    }
    ...
    for name, param_grid in param_grids.items():
        ...
        for i in range(base_count):
            # Bootstrap sampling for each base classifier
            X_train_sample, y_train_sample = resample(X_train, y_train, n_samples=2*len(X_train),random_state=42 + i)

            # Initialize model with imbalance handling parameters
            model = model_class(
                **imbalance_params.get(name, {}),
                verbose=-1 if name != 'CAT' else 0,
                random_state=42 + i
            )

            if run_flag:
                model = perform_grid_search(model, param_grid, X_train_sample, y_train_sample)

            model.fit(X_train_sample, y_train_sample)
            base_classifiers.append(model)
            print(f"Trained {name} model {i + 1}/{base_count}")

    sys.stdout.write("Base classifiers trained with bootstrap sampling and imbalance handling if specified!\n")
...
def predict_and_save(stack, test_data_path, final_features, threshold, output_path):
    ...
    test_data = clean_feature_names(test_data).round(4)  # Adjust precision
...
def main():
    ...
    base_classifiers = get_base_classifiers(X_train, y_train, args.run, args.base_count, args.use_imbalance)
